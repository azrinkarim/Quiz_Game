...
const Navbar = () => {
  ...
  const renderDesktopAuthButton = () => {
    return isAuthenticated ? (
      <button
        ...
        className='px-3 md:px-3 lg:px-6 py-1.5 md:py-2 lg:py-3 
        bg-gradient-to-br from-amber-600 to-amber-700 text-[#2D1B0E] 
        rounded-2xl font-bold hover:shadow-amber-600/40 transition-all 
        transform hover:scale-[1.02] border-2 border-amber-600/20 
        flex items-center space-x-2 shadow-md shadow-amber-900/20 
  ...
  const renderMobileAuthButton = () => {
    return isAuthenticated ? (
      <button
        onClick={handleLogout}
        className='w-full px-4 py-3 bg-gradient-to-br from-amber-500 
        to-amber-700 text-[#2D1B0E] rounded-xl font-semibold flex 
  ...
  return (
    <nav className='bg-[#2D1B0E] border-amber-900/30 shadow-amber-900/30 sticky top-0 z-50 shadow-[0_25px_-12px] font-vibes group/nav overflow-x-hidden'>
      ...
      <div className='max-w-7xl mx-auto px-4 relative'>
        <div className='flex justify-between items-center h-16 md:h-20 lg:h-24'>
          ...
          <div className='hidden md:flex items-center space-x-2 md:space-x-1 lg:space-x-4 flex-1 justify-end'>
            ...
            <div className='flex items-center space-x-2 md:space-x-3 lg:space-x-4 ml-3 md:ml-3 lg:m-6 mr-2 md:mr-3 lg:me-4'>
              <NavLink
                to='/cart'
                className='relative p-2 md:p-2.0 lg:p-3 text-amber-100 rounded-xl transition-all
          ...
          <div className='md:hidden flex items-center mr-2'>
            <button
              ...
              p-2 rounded-xl border-2 border-amber-900/30 hover:border-amber-600/50 shadow-md shadow-amber-900/20
      ...
      {/* Mobile menu */}
      {isOpen && (
        <div className='md:hidden bg-[#2D1B0E] border-t-4 border-amber-99/40 relative shadow-lg shadow-amber-900/30 w-full'>
          <div className='px-4 py-4 space-y-2'>
            ...
            <div className='pt-4 border-t-2 border-amber-900/30 space-y-2'>
              <NavLink
                ...
                items-center justify-center text-sm relative'
              >
                <FiShoppingCart className='text-lg' />
                {totalItems > 0 && (
                  <span className='absolute top-1 right-1 bg-amber-600 text-amber-100 text-xs w-5 h-5 rounded-full flex items-center justify-center'>
                    {totalItems}
                  </span>
                )}
              </NavLink>
